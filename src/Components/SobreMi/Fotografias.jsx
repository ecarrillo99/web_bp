...
const SlickSlider = lazy(() => {
  ...
  return Promise.all([
    ...
    import("react-slick")
  ]).then(([css1, css2, sliderModule]) => {
    return { default: sliderModule.default };
  });
});
...
const LoadingSpinner = () => (
  <div className="flex justify-center items-center p-10">
    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#96c121]"></div>
  </div>
);
...
const ImageModal = lazy(() => Promise.resolve({
  default: ({ src, isOpen, onClose, onNext, onPrev, images }) => {
...
const GalleryCard = lazy(() => Promise.resolve({
  default: ({ src, index, activeSlide, descriptions, openModal }) => (
    ...
    <div id="galeria" className="py-8">
      <div className="mb-8 max-w-3xl">
        <h2 className='text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#005F6B] to-[#96c121]'>
          Bernardo Polo Andrade en Acción
        </h2>
        ...
        <Suspense fallback={<LoadingSpinner />}>
          <SlickSlider {...settings}>
            {images.map((src, index) => (
              <Suspense key={index} fallback={<div className="px-3 py-2 h-64 bg-gray-100 rounded-xl animate-pulse"></div>}>
                <GalleryCard 
                  src={src} 
                  index={index} 
